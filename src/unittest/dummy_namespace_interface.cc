...
void dummy_performer_t::write(const write_t &write,
    ...
    store->write(
            ...
            write, response, write_durability_t::SOFT, timestamp, &token, &non_interruptor);
}
...
void dummy_timestamper_t::write(const write_t &write, write_response_t *response, order_token_t otok) THROWS_NOTHING {
    ...
    next->write(write, response, current_timestamp);
}
    ...
    : ctx(_ctx)
{
    ...
    for (size_t i = 0; i < shards.size(); ++i) {
        ...
        timestampers.push_back(make_scoped<dummy_timestamper_t>(performers.back().get()));
