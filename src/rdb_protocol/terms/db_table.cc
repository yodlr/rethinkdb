...
public:
    meta_op_term_t(compile_env_t *env, const protob_t<const Term> term,
        ...
        : op_term_t(env, term, bt, std::move(argspec), std::move(optargspec)) { }
...
public:
    db_term_t(compile_env_t *env, const protob_t<const Term> &term,
              backtrace_id_t bt)
        : meta_op_term_t(env, term, bt, argspec_t(1)) { }
private:
...
public:
    table_create_term_t(compile_env_t *env, const protob_t<const Term> &term,
                        backtrace_id_t bt)
        : meta_op_term_t(env, term, bt, argspec_t(1, 2),
...
public:
    db_list_term_t(compile_env_t *env, const protob_t<const Term> &term,
                   backtrace_id_t bt)
        : meta_op_term_t(env, term, bt, argspec_t(0)) { }
private:
...
public:
    table_list_term_t(compile_env_t *env, const protob_t<const Term> &term,
                      backtrace_id_t bt)
        : meta_op_term_t(env, term, bt, argspec_t(0, 1)) { }
private:
    virtual scoped_ptr_t<val_t> eval_impl(scope_env_t *env, args_t *args, eval_flags_t) const {
...
public:
    config_term_t(compile_env_t *env, const protob_t<const Term> &term,
                  backtrace_id_t bt)
        : meta_op_term_t(env, term, bt, argspec_t(1, 1), optargspec_t({})) { }
private:
    virtual scoped_ptr_t<val_t> eval_impl(scope_env_t *env, args_t *args, eval_flags_t) const {
...
public:
    table_or_db_meta_term_t(compile_env_t *env, const protob_t<const Term> &term,
                            backtrace_id_t bt, optargspec_t &&optargs)
        /* None of the subclasses take positional arguments except for the table/db. */
        : meta_op_term_t(env, term, bt, argspec_t(0, 1), std::move(optargs)) { }
protected:
...
public:
    wait_term_t(compile_env_t *env, const protob_t<const Term> &term,
                backtrace_id_t bt)
        : table_or_db_meta_term_t(env, term, bt,
...
public:
    rebalance_term_t(compile_env_t *env, const protob_t<const Term> &term,
                     backtrace_id_t bt)
        : table_or_db_meta_term_t(env, term, bt, optargspec_t({})) { }
private:
    virtual scoped_ptr_t<val_t> eval_impl_on_table_or_db(
...
public:
    table_term_t(compile_env_t *env, const protob_t<const Term> &term,
                 backtrace_id_t bt)
        : op_term_t(env, term, bt, argspec_t(1, 2),
...
public:
    get_all_term_t(compile_env_t *env, const protob_t<const Term> &term,
                   backtrace_id_t bt)
        : op_term_t(env, term, bt, argspec_t(2, -1), optargspec_t({ "index" })) { }
private:
...
counted_t<term_t> make_db_term(
    ...
    return make_counted<db_term_t>(env, term, bt);
}

counted_t<term_t> make_table_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<table_term_t>(env, term, bt);
}

counted_t<term_t> make_get_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<get_term_t>(env, term, bt);
}

counted_t<term_t> make_get_all_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<get_all_term_t>(env, term, bt);
}

counted_t<term_t> make_db_create_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<db_create_term_t>(env, term, bt);
}

counted_t<term_t> make_db_drop_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<db_drop_term_t>(env, term, bt);
}

counted_t<term_t> make_db_list_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<db_list_term_t>(env, term, bt);
}

counted_t<term_t> make_table_create_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<table_create_term_t>(env, term, bt);
}

counted_t<term_t> make_table_drop_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<table_drop_term_t>(env, term, bt);
}

counted_t<term_t> make_table_list_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<table_list_term_t>(env, term, bt);
}

counted_t<term_t> make_config_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<config_term_t>(env, term, bt);
}

counted_t<term_t> make_status_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<status_term_t>(env, term, bt);
}

counted_t<term_t> make_wait_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<wait_term_t>(env, term, bt);
}

counted_t<term_t> make_reconfigure_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<reconfigure_term_t>(env, term, bt);
}

counted_t<term_t> make_rebalance_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<rebalance_term_t>(env, term, bt);
}

counted_t<term_t> make_sync_term(
        compile_env_t *env, const protob_t<const Term> &term, backtrace_id_t bt) {
    return make_counted<sync_term_t>(env, term, bt);
}
